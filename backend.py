...
def api_describe_media():
    ...
    try:
        ...
        if media_type == 'image':
            ...
            mime = 'image/jpeg'
            if context.strip():
                prompt = f"Baseado na transcrição a seguir do áudio desta vistoria: '{context}', descreva de forma técnica o que você vê especificamente NESTA imagem para servir como uma legenda curta (focando no cômodo e nas patologias). Se não houver patologias aparentes, apenas omita essa informação (não escreva que não há patologias). Escreva apenas 1 frase direta com no máximo 100 caracteres."
            else:
                prompt = "Descreva de forma técnica e objetiva o que você vê nesta imagem de vistoria, em 1 frase curta com no máximo 100 caracteres. Se não houver patologias aparentes, apenas omita essa informação. Não divague."
        else:
            mime = 'audio/mp4'
